            ...
            {SideBarItems.map((item) => (
              <Link
                ...
                className={`flex items-center gap-2 my-3 rounded-md text-lg font-medium transition-colors ${
                  pathname === item.href
                    ? "bg-secondary-800 text-white my-3 ps-2 h-[68px]"
                    : "transition-all duration-600 hover:text-white hover:bg-secondary-800 ps-4 hover:ps-2 h-[56px] hover:h-[68px] hover:text-2xl text-neural-800"
                }`}
              >
                {item.icon}
                {item.label}
